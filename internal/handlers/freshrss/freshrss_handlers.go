...
func HandleSyncFeed(h *core.Handler, w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		if err == nil {
			...
			lastSyncErrors.Store(key, err.Error())
		}
...
func HandleSyncStatus(h *core.Handler, w http.ResponseWriter, r *http.Request) {
	...
	response.JSON(w, map[string]interface{}{
		...
		"syncing":         isSyncing(h.DB, requestProvider(r)),
		"error":           syncError(h.DB, requestProvider(r)),
	})
}

func isSyncing(db *database.DB, provider string) bool {
	_, ok := activeSyncs.Load(syncKey{db, provider})
	return ok
}
func syncError(db *database.DB, provider string) string {
	value, ok := lastSyncErrors.Load(syncKey{db, provider})
	if ok {
		return value.(string)
	}
	return ""
