...
func (s *BidirectionalSyncService) Sync(ctx context.Context) (*SyncResult, error) {
	...
	if s.client.provider == ProviderFreshRSS {
		pulled, pushed, err := s.syncIncremental(ctx)
		result.PullChangesCount, result.PushChangesCount = pulled, pushed
		result.PullSuccess, result.PushSuccess = err == nil, err == nil
		if err != nil {
