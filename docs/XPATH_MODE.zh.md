...
在新增/编辑订阅的 XPath 表单中输入 HTTP(S) 网址，点击“可视化 XPath 选择器”。
先点击一篇文章的标题，选择器会建议文章列表，并用绿框标出匹配区块。
检查范围后确认，自动填入标题和链接；也可手动选择容器、通过父级路径调整。
如果列表推断不准确，可再选另一篇文章的标题来校准。
标题、链接、日期、摘要和缩略图都支持第二示例校准：先选择字段，再点击“再选一个示例来校准”，
在另一篇文章中点击同一字段。无法得到明确的共同规则时会提示，取消校准保留原规则。
每个字段显示提取覆盖数量，结果预览随选取更新，XPath 表达式在高级区域中查看。
检查结果后应用，日期格式仍在表单中设置；提交订阅表单前不会保存。

隔离预览保留原网页样式和图片，禁用原网页脚本、嵌入页面、表单提交和链接跳转。
HTML 抓取使用表单中的代理配置；样式、图片和字体使用 WebView 的网络配置。
依赖 JavaScript 或登录的页面可能显示不完整，动态内容建议使用自定义脚本。
生成路径依据当前网页结构，站点调整结构后可能需要重新选择。
