...
const zh: TranslationMessages = {
  article: {
    ...
    action: {
      ...
      markAboveInFeedAsRead: '将本订阅源以上文章标记为已读',
      markBelowInFeedAsRead: '将本订阅源以下文章标记为已读',
      markRelativeInFeedConfirmMessage: '确定将当前订阅源中此方向的文章标记为已读吗？',
      ...
      markedSelectedAsRead: '已将选中的 {count} 篇文章标记为已读',
      markedSelectedAsUnread: '已将选中的 {count} 篇文章标记为未读',
      batchReadUpdateFailed: '更新所选文章失败',
      noArticlesToMark: '没有可标记的文章',
      selectArticles: '多选文章',
      selectArticle: '选择文章',
      deselectArticle: '取消选择文章',
      selectAllVisible: '选择当前全部文章',
      selectedArticles: '已选择 {count} 篇',
      openArticle: '打开文章',
    ...
    table: {
      ...
      read: '已读',
      unread: '未读',
      showColumn: '显示「{name}」列',
      resize: '调整文章列表与正文区域大小',
    },
    navigation: {
  ...
  modal: {
    ...
    feed: {
      previewTitle: '预览订阅源',
      previewHint: '订阅前先浏览最新文章。预览不会保存订阅，也不会改变阅读状态。',
      previewLoading: '正在加载订阅预览…',
      previewFailed: '无法加载订阅源，请检查地址和代理设置后重试。',
      previewRetry: '重新预览',
      previewCount: '显示 {total} 篇可用文章中的最新 {count} 篇。',
      previewEmpty: '此订阅源暂时没有文章，你仍可订阅以接收后续更新。',
      previewUntitled: '无标题文章',
      previewNoContent: '订阅源未提供正文，可打开原网页阅读。',
      previewTruncated: '这篇长文章的预览内容已截短，可打开原网页继续阅读。',
      ...
      picker: {
        ...
        incompatibleExamples:
          '无法生成明确的共同规则。请在另一篇文章中选择同一字段，或取消校准以保留原规则。',
        firstExample: '第一个标题：',
        secondExample: '第二个标题：',
        calibrate: '再选一个标题来校准',
        calibrationHint: '现在请点击同一列表中另一篇文章的标题。',
        clearCalibration: '仅使用第一个标题',
        stepTitle: '① 点击文章标题　② 确认文章列表　③ 检查提取结果',
        stepFields: '已选择文章列表，可直接应用或补充可选字段',
        previewAbout: '预览说明',
        pageInstruction: '悬停查看 · 点击选中 · Enter 确认',
        highlightLegend: '蓝框：当前元素 · 绿框：匹配文章',
        startTitle: '先点击网页中的一篇文章标题',
        startHint: '自动识别重复的文章列表，并为你填写标题和链接。',
        suggested: '发现可能的文章列表，共 {count} 篇',
        confirmGroupHint: '检查绿色边框是否分别圈住一篇文章，确认没有混入导航或空白行。',
        otherGroup: '尝试其他候选容器',
        noSuggestion: '暂未找到可靠的文章列表。可换一个标题，或手动选择文章容器。',
        manual: '手动选择文章容器',
        fields: '文章字段',
        changeGroup: '重新选择文章列表',
        picked: '已选择 · 点击修改',
        required: '必选',
        optional: '可选',
        clearField: '清除{field}',
        confirmGroup: '使用这个文章列表',
        results: '提取结果预览',
        ready: '已可应用，也可在上方补充其他字段。',
        advanced: '高级：生成的 XPath',
        applyHint: '应用到订阅表单后，保存表单才会生效。',
        needFields: '选好标题和链接后即可应用。',
        title: '可视化 XPath 选择器',
        hint: '点击网页元素，通过父级路径选择文章容器，再选择字段并检查提取示例。',
        staticHint:
          '隔离预览保留网页样式和图片，禁用原网页脚本、表单和跳转。仅由 JavaScript 生成的内容需要自定义脚本；应用后请设置日期格式。',
        item: '文章容器',
        titleField: '标题',
        uri: '链接',
        timestamp: '日期',
        content: '摘要',
        thumbnail: '缩略图',
        selectElement: '悬停高亮，点击选择网页元素。',
        choose: '用于当前字段',
        withinItem:
          '可在任一匹配文章内选择字段。选择带链接的标题会自动填写链接。日期、摘要和图片可不选。',
        matches: '匹配 {count} 个容器',
        apply: '应用 XPath',
        retry: '重新加载预览',
        loadFailed: '无法加载网页，请检查网址、代理和网络。',
        selecting: '正在选择：{field}',
        containerHint:
          '选择包含单篇文章的重复区块。点击父级路径可以扩大选区，请检查示例中是否混入导航或空白行。',
        ancestors: '元素父级路径',
        invalidElement: '请在匹配文章内部选择；链接需选择带链接的元素，缩略图需选择图片。',
        clear: '清除',
        validSamples: '{total} 篇匹配文章中，{count} 篇具有标题和链接。',
        pagePreview: '隔离网页预览',
      },
  ...
  setting: {
    ...
    typography: {
      ...
      fontHyperlegible: '无障碍',
      fontSerif: '衬线字体',
      fontSerifDefault: '默认衬线',
      fontSystem: '系统字体',
      fontSystemDefault: '系统默认',
      readerMaxWidth: '正文最大宽度',
      readerMaxWidthDesc: '将长文居中并保持舒适的每行字数',
    },
    database: {
      dataDirectory: '数据目录',
    ...
    reading: {
      ...
      showReadingTimeDesc: '在文章信息栏显示估算阅读时长',
