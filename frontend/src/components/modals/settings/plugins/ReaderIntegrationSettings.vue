...
async function status() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      syncing.value = Boolean(data.syncing);
...
async function syncNow() {
  if (syncing.value || startingSync.value) return;
  ...
  <div>
    ...
    <NestedSettingsContainer v-if="enabled">
      ...
      <SubSettingItem :icon="PhCloudCheck" :title="t('setting.freshrss.syncNow')">
        ...
        <button class="btn-secondary" :disabled="syncing || startingSync" @click="syncNow">
          <PhArrowClockwise :size="16" :class="{ 'animate-spin': syncing || startingSync }" />
